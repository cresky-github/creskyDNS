...
log:
  enabled: true                          # 是否启用日志
  path: "./logs/windspliving.log"        # 日志文件路径
  level: "info"                          # 日志级别
  max_time: 3d                           # 最大保存时间
...
```log
|2026-01-10|14:35:42,123|TRACE|windspliving|dns_resolver|进入函数: resolve_domain()|
|2026-01-10|14:35:42,124|TRACE|windspliving|dns_resolver|参数: domain=google.com, record_type=A|
|2026-01-10|14:35:42,125|TRACE|windspliving|rule_matcher|开始匹配规则...|
```

#### DEBUG 级别

```log
|2026-01-10|14:35:42,130|DEBUG|windspliving|cache|缓存查询: google.com → 未命中|
|2026-01-10|14:35:42,135|DEBUG|windspliving|upstream|选择上游: ali_dns (https://dns.alidns.com)|
|2026-01-10|14:35:42,145|DEBUG|windspliving|dns_resolver|解析结果: 142.250.185.68|
```

#### INFO 级别

```log
|2026-01-10|14:35:42,000|INFO|windspliving|main|DNS 转发器启动成功|
|2026-01-10|14:35:42,050|INFO|windspliving|listener|监听器 'main' 绑定端口: 5353|
|2026-01-10|14:35:43,000|INFO|windspliving|list_loader|域名列表 'direct' 加载成功: 1245 个域名|
```

#### WARN 级别

```log
|2026-01-10|14:38:00,234|WARN|windspliving|cache|缓存使用率 95%，接近上限|
|2026-01-10|14:39:15,456|WARN|windspliving|upstream|上游 DNS 响应慢: 1500ms|
|2026-01-10|14:40:30,789|WARN|windspliving|list_loader|域名列表格式异常，已跳过 3 行|
```

#### ERROR 级别

```log
|2026-01-10|14:40:12,567|ERROR|windspliving|upstream|上游 DNS 连接失败: timeout after 5s|
|2026-01-10|14:41:25,890|ERROR|windspliving|listener|监听器端口冲突: 5353 已被占用|
|2026-01-10|14:42:00,123|ERROR|windspliving|config|配置文件解析失败: invalid YAML syntax|
```
...
- 旧文件重命名：`windspliving.log.2026-01-10`
...
- 旧文件重命名：`windspliving.log.1`, `windspliving.log.2`
...
**文件命名示例**：
```
windspliving.log              # 当前日志
windspliving.log.2026-01-10   # 按日期轮转的备份
windspliving.log.1            # 按序号轮转的备份
windspliving.log.2
windspliving.log.3
```
...
```bash
# Linux/macOS
tail -f logs/windspliving.log

# Windows PowerShell
Get-Content logs/windspliving.log -Wait -Tail 20
```
...
```bash
# 查找所有错误
grep "|ERROR|" logs/windspliving.log

# 查找特定模块的错误
grep "|ERROR|.*|upstream|" logs/windspliving.log
```
...
```bash
# 统计今天的查询次数
grep "2026-01-10" logs/windspliving.log | grep "查询:" | wc -l

# 按小时统计
grep "2026-01-10" logs/windspliving.log | cut -d'|' -f2 | cut -d':' -f1 | sort | uniq -c
```
...
```bash
# 提取所有响应时间日志
grep "响应时间:" logs/windspliving.log | cut -d'|' -f6
```
...
```bash
awk -F'|' '{print $3}' logs/windspliving.log | sort | uniq -c
```
...
```bash
awk -F'|' '$2 >= "14:00:00" && $2 <= "15:00:00"' logs/windspliving.log
```

#### 3. 统计各模块日志数量

```bash
awk -F'|' '{print $5}' logs/windspliving.log | sort | uniq -c | sort -rn
```
...
# 统计错误类型
errors = Counter()
for entry in parse_log('logs/windspliving.log'):
...
input {
  file {
    path => "/path/to/logs/windspliving.log"
...
```bash
# Linux/macOS
less logs/windspliving.log
tail -n 100 logs/windspliving.log

# Windows
Get-Content logs/windspliving.log -Tail 100
```

### Q4: 如何动态修改日志级别？

**A**: 目前需要修改配置文件并重启。未来版本可能支持热重新加载或 API 动态调整。
